...
namespace Warehouse.API.Controllers
{
    ...
    public class StockTakingItemsController : ControllerBase
    {
        ...
        public async Task<ActionResult<StockTakingDTO.ItemDTO>> GetMoveToBin(int stockTakingId, long positionId)
        {
            try
            {
                StockTaking.Item entity = await this.Mediator.Send(new MoveStockTakingItemToBinCommand(stockTakingId, positionId, false));
        ...
        // GET: api/StockTakingItems/Restore/1/20
        [HttpGet("Restore/{stockTakingId}/{positionId}")]
        [ProducesResponseType(200)]
        [ProducesResponseType(404)]
        [ProducesResponseType(409)]
        [ProducesResponseType(500)]
        public async Task<ActionResult<StockTakingDTO.ItemDTO>> GetRestore(int stockTakingId, long positionId)
        {
            try
            {
                StockTaking.Item entity = await this.Mediator.Send(new RestoreStockTakingItemFromBinCommand(stockTakingId, positionId));
                return this.Ok(this.Mapper.Map<StockTakingDTO.ItemDTO>(entity));
            }
            catch (EntityNotFoundException ex)
            {
