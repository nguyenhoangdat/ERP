...
namespace Warehouse.Domain.Tests.Entities
{
    ...
    public class StockTakingTests
    {
        ...
        public void CanBeMovedToBin()
        {
            ...
            stockTaking = this.DatabaseContext.StockTakings.FirstOrDefault(x => x.Id == 1);
            Assert.IsFalse(stockTaking.CanBeMovedToBin());

            stockTaking = this.DatabaseContext.StockTakings.FirstOrDefault(x => x.Id == 2);
            Assert.IsTrue(stockTaking.CanBeMovedToBin());
        }
        [TestMethod, TestCategory("Extensions")]
        public void CanBeRestoredFromBin()
        {
            ...
            stockTaking = this.DatabaseContext.StockTakings.FirstOrDefault(x => x.Id == 1);
            Assert.IsFalse(stockTaking.CanBeRestoredFromBin());

            stockTaking = this.DatabaseContext.StockTakings.FirstOrDefault(x => x.Id == 3);
            Assert.IsTrue(stockTaking.CanBeRestoredFromBin());
        }
    }
}
